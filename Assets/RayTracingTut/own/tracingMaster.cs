...
public class tracingMaster : MonoBehaviour {
    ...
    private static List<ObjComponent> rayTracingObjects = new List<ObjComponent>();
    ...
    public static void RegisterObject(ObjComponent obj) {
        rayTracingObjects.Add(obj);
        meshObjectsNeedRebuilding = true;
    }
    public static void UnregisterObject(ObjComponent obj) {
    ...
    private void setComputeBuffer(string name, ComputeBuffer buffer) {
        if (buffer != null) {
            this.RayTracingShader.SetBuffer(0, name, buffer);
            Debug.Log("Set buffer " + name);
            Debug.Log("Buffer count: " + buffer.count);
            Debug.Log(this.RayTracingShader.name);
        } else {
            Debug.LogError("Buffer " + name + " is null");
        }
    }
    ...
    private void RebuildMeshObjectBuffers() {
        ...
        // loop over all the objects and gather their data
        for (int i = 0; i < rayTracingObjects.Count; ++i) {
            ObjComponent obj = rayTracingObjects[i];
    ...
    private void setShaderParameters() {
        ...
        this.setComputeBuffer("spheres", this.sphereBuffer);
        this.setComputeBuffer("meshObjects", this.meshObjectBuffer);
        this.setComputeBuffer("vertices", this.vertexBuffer);
        this.setComputeBuffer("indices", this.indexBuffer);
    }
